...
\eQb
B &=& \text{Res}_{z = i} [f(z)e^{iaz}] = \dfrac{e^{iaz}}{z+i}]_{z=i}\\
&=& \dfrac{e^{-a}}{2i}.
\eQe
By the residue theorem, it follows that
\eQb
\int_{-R}^{R} \dfrac{\cos(ax)}{x^2+1}dx &=& 
\pi e^{-a} - \text{Re} \int_{C_R} f(z)e^{iaz}dz.
\eQe
Observe that
\eQb
\left| \text{Re} \int_{C_R} f(z)e^{iaz}dz \right|
&\leq& \left| \int_{C_R} f(z)e^{iaz}dz \right| \\
&\leq& \dfrac{\pi R}{R^2-1}. \\
\eQe
As $\dfrac{\pi R}{R^2 - 1}$ tends to $0$ as $R \to \infty$,
by the Jordan's Lemma, we obtain 
\eQb
\int_{-\infty}^{\infty} \dfrac{\cos(ax)}{x^2+1}dx &=& \pi e^{-a}.
\eQe
\hfill $\qed$
\end{solution}
...
\eQb
\text{Res}_{z=i} [f(z)e^{iz}] &=& \dfrac{z^3 e^{iz}}{(z+i)(z^2+9)}]_{z=i} \\
&=& -\dfrac{1}{16e}, \\
\text{Res}_{z=3i} [f(z)e^{iz}] &=& \dfrac{z^3e^{iz}}{(z^2+1)(z+3i)}_{z=3i} \\
&=& \dfrac{9}{16e^3}. \\ 
\eQe 
By the residue theorem and the Jordan's Lemma, applied as above, we have
\eQb
\int_{-\infty}^{\infty} \dfrac{x^3\sin(x)}{(x^2+1)(x^2+9)} dx 
&=& \dfrac{\pi}{8e}(\dfrac{9}{e^2}-1). \\
\eQe
\hfill $\qed$
\end{solution}
\end{document}
